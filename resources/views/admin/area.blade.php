...
<head>
    ...
    <title>Kelola Area — Kopgun Admin</title>
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-[220px] flex-shrink-0 bg-[#1A1A2E] border-r border-[#2A2A48] flex flex-col">
        ...
        <nav class="flex-1 px-3 py-4 overflow-y-auto space-y-0.5">
            ...
            <a href="{{ route('admin.area.index') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] font-semibold bg-[#2C4870]/30 text-[#9DBADD] border border-[#2C4870]/50">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-6 flex-shrink-0">
            <div>
                <h1 class="text-[15px] font-semibold text-gray-900">Kelola Area</h1>
                <p class="text-[12px] text-gray-400">Kelola foto & informasi Area Kopgun</p>
            </div>
        </header>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4 items-start">
                ...
                <div class="bg-white rounded-xl border border-gray-100">
                    <div class="flex items-center gap-2.5 px-5 py-3.5 border-b border-gray-100">
                        ...
                        <h2 class="text-[13px] font-semibold text-gray-800">Tambah Area Baru</h2>
                    </div>
                    <div class="p-5">
                        @if(session('success'))
                        ...
                        <form action="{{ route('admin.area.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-4">
                                <label class="block text-[12px] font-medium text-gray-600 mb-1.5">Foto Area <span class="text-red-400">*</span></label>
                            ...
                            <div class="mb-5">
                                <label class="block text-[12px] font-medium text-gray-600 mb-1.5">Deskripsi Area <span class="text-gray-400 font-normal">(Opsional)</span></label>
                                <textarea name="deskripsi" rows="4" placeholder="Ceritakan tentang Area ini..." class="w-full text-[13px] px-3 py-2 rounded-lg border border-gray-200 bg-white text-gray-800 placeholder-gray-300 focus:outline-none focus:border-[#2C4870] focus:ring-2 focus:ring-[#E7EEF6] transition resize-none">{{ old('deskripsi') }}</textarea>
                ...
                <div class="bg-white rounded-xl border border-gray-100">
                    <div class="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
                        <h2 class="text-[13px] font-semibold text-gray-800">Daftar AREA</h2>
                        <span class="text-[11px] text-gray-400 bg-gray-50 px-2.5 py-1 rounded-full border border-gray-100">{{ $areas->count() }} item</span>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse($areas as $area)
                                    <tr class="menu-row hover:bg-gray-50/50 transition-colors">
                                        <td class="px-5 py-3.5">
                                            <img src="{{ asset('storage/' . $area->foto) }}" class="w-16 h-16 object-cover rounded-lg border border-gray-100">
                                        </td>
                                        <td class="px-5 py-3.5">
                                            <p class="text-[12px] text-gray-600 line-clamp-3">{{ $area->deskripsi ?: 'Tidak ada deskripsi.' }}</p>
                                        </td>
                                        <td class="px-5 py-3.5 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                <button type="button"
                                                    data-id="{{ $area->id }}"
                                                    data-deskripsi="{{ $area->deskripsi }}"
                                                    data-foto="{{ asset('storage/' . $area->foto) }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="px-5 py-16 text-center text-[12px] text-gray-400">Belum ada area yang ditambahkan.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div class="relative z-10 flex justify-center px-4 py-10 min-h-full">
        <div class="bg-white rounded-xl border border-gray-100 shadow-2xl w-full max-w-md h-fit">
            <div class="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 bg-gray-50/50 rounded-t-xl">
                <h2 class="text-[13px] font-semibold text-gray-800">Ubah Data Area</h2>
                ...
                <div class="px-5 py-4 space-y-4">
                    <div>
                        <label class="block text-[12px] font-medium text-gray-600 mb-1.5">
                            Foto Area <span class="text-gray-400 font-normal">(kosongkan jika tidak diganti)</span>
                        </label>
                        <div class="mb-2">
                    ...
                    <div>
                        <label class="block text-[12px] font-medium text-gray-600 mb-1.5">Deskripsi Area</label>
...
<script>
    ...
    function openEditModal(btn) {
        document.getElementById('editForm').action = '/admin/area/' + btn.dataset.id;
