...
<section class="max-w-6xl mx-auto px-4 md:px-8 py-14">
    <h1 class="text-2xl md:text-3xl font-extrabold text-center tracking-tight mb-10">EVENT KOPGUN</h1>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($events as $event)
            <div class="rounded-2xl overflow-hidden shadow-lg border border-gray-100">
                <img src="{{ asset('storage/' . $event->foto) }}" class="w-full aspect-video object-cover">
                @if($event->deskripsi)
                    <div class="p-4">
                        <p class="text-[13px] text-gray-600 leading-relaxed">{{ $event->deskripsi }}</p>
                    </div>
                @endif
            </div>
        @empty
            <div class="col-span-full py-16 text-center text-[13px] text-gray-400">Belum ada event yang tersedia saat ini.</div>
        @endforelse
    </div>
</section>
